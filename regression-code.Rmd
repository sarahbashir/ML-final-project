...
set.seed(47)
trainingIndex <- sample(1:nrow(war_Data), 0.8*nrow(war_Data)) #row indeces for training
trainData <- war_Data[trainingIndex, ] #training data
testData <- war_Data[-trainingIndex, ] #test data

#build model on training data
linMod <- lm(MaxTemp ~ MinTemp, data=war_Data)
...
#cross validation
#need to check 2 things from k fold predictions
#if each of k-fold model's prediction accuracy doesn't very too much for any one sample
#and if lines of best fit fromt he k-folds don't cary too much with respect to the slope and level
#(line sshould be parallel and as close to each other as possible)

war_Data<-as.data.frame(war_Data)

crossv <- suppressWarnings(CVlm(df=war_Data, form.lm=MaxTemp ~ MinTemp, m = 5, dots = FALSE, seed = 4, legend.pos = "topleft", printit = FALSE, main = "Small symbols are predicted and big are actual results"));

attr(crossv, 'ms') #mean squared error
