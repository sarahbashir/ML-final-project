...
#in sample accuracy
#need to convert log odds to a probability
poisMod <- glm(Precipitation ~ Total, data = BR.train, family = poisson)
poisPrediction <- predict(poisMod, BR.train) #predict total number of bikes
poisPred <- gtools::inv.logit(poisPrediction)
...
```
We see that a higher mean temperature means more bikes while a higher level of preciptation means a lower amount of bikes (not a very good model, however).
